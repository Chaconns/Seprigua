document.addEventListener("DOMContentLoaded", () => {
  ...
  if ("IntersectionObserver" in window) {
    const navObs = new IntersectionObserver(
      ...
      { threshold: 0.25, rootMargin: "-15% 0px -55%" },
    );
  ...
  if (track && slides.length) {
    slides.forEach((slide, i) => {
      ...
      dot.setAttribute("aria-label", `Mostrar servicio ${i + 1}`);
  ...
  form?.addEventListener("submit", (e) => {
    ...
    if (msg) {
      ...
      msg.textContent =
        "Mensaje preparado correctamente. La conexión con el backend se integrará en el siguiente incremento.";
    }
    form.reset();
    setTimeout(() => {
  ...
  /* Chatbot reservado para un incremento futuro: solo vista, sin funcionalidad. */
