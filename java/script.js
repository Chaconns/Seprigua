document.addEventListener("DOMContentLoaded",()=>{
  ...
  if("IntersectionObserver" in window){
    const navObs=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting)navLinks.forEach(a=>a.classList.toggle("active",a.hash===`#${e.target.id}`))}),{threshold:.25,rootMargin:"-15% 0px -55%"}); sections.forEach(s=>navObs.observe(s));
